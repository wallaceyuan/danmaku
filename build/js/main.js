...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        //this.$element.append('<video id="tenvideo_video_player_0" class="danmu-video"  x-webkit-airplay="true" webkit-playsinline="true" preload="none"  src="' + options.src + '" width="' + options.width + '" height="' + options.height + '"></video>');
      // this.$element.append('<div class="danmu-video"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" name="player1" id="player1" class="danmu-video"  webkit-playsinline="webkit-playsinline" > <param name="movie" value="http://player.kksmg.com/data/player_swf/KKPlayer.swf"> <param name="flashvars" value="playerId=2969363206&amp;liveChannelID=144"> <param name="allowFullScreen" value="true"> <param name="wmode" value="transparent"> <param name="allowscriptaccess" value="always"> <embed src="http://player.kksmg.com/data/player_swf/KKPlayer.swf" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" wmode="opaque" name="player2" id="player2" flashvars="playerId=2969363206&amp;liveChannelID=144"> </object></div>');
...
*/        this.$ctrlMain.append('<div class="show-danmu  ctrl-btn-right ctrl-btn-right-active"><span class="glyphicon glyphicon-comment" aria-hidden="true"></span></div>');
        ...
        /*PC*/
        if(!aaa){
            $('.ctrl-progress').css('display','none');
            $('.play-btn').css('display','none');
            $(" .danmu-div").danmu('danmuResume');
        }else{
            //播放器状态
            that.video = $(that.id + " .danmu-video").get(0);
            that.current = 0;  //当前播放时间
            that.duration = that.video.duration;  //总时间
...
/*        this.video = $(this.id + " .danmu-video").get(0);
        ...
        this.getDanmu = function () {
            ...
            $.get(that.options.urlToGetDanmu+'?time='+time, function (data, status) {
                console.log(data);
        ...
        this.sendDanmu = function (e) {
            ...
            textObj = '{ "text":"' + text + '","color":"' + color + '","size":"' + size + '","position":"' + position + '","time":' + pushtime + ',"isnew":""}';
            var newObj = eval('(' + textObj + ')');

            $(e.data.that.id + " .danmu-div").danmu("addDanmu", newObj);
            
            $(that.id + ' .danmu-div').danmu("pushCach", time);
...
/*        var mainTimer = setInterval(function () {
        ...
        var secTimer = setInterval(function () {
           // if (Math.abs($(that.id + " .danmu-div").data("nowTime") - parseInt(that.video.currentTime*10)) > 1) {
            ...
            // $(that.id + " .danmu-div").data("nowTime", parseInt(that.video.currentTime*10));
           $(that.id + " .danmu-div").data("nowTime", parseInt($.now()/1000));
          //  }
        }, 50);
        var _this = this;
